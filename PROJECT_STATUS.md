...
> ## 🚧 OPEN TODO (raised 2026-09-18, NEXT RELEASE) — Part-type recategorization
>
> MPP (phone call, 2026-09-18) uses five part categories: **MPP Cast**, **Components** (purchased), **Pass Through**, Sub-Assembly, Finished Good. What prod calls `PassThrough` today (the 33 parts `0089` retyped) are really **Components**; the true pass-through parts are the vendor-cast / vendor-machined finished goods in `reference/MPP pass through parts.pdf` (scanned, upside down: MPP + DSI tables and "New Model"; LMC / METTS / Hailin / DSI suppliers). The code keys on type **codes**, so the 2026-09-18 release behaves correctly on today's data -- only the labels are wrong.
>
> **Plan:** new code `Purchased` named *Components* (retype the 33 current `PassThrough` items; switch the code references `PassThrough` -> `Purchased`); `PassThrough` renamed *Pass Through* and receives the parts in the PDF; `Component` keeps its code and is named *MPP Cast*. Touches `Lot_GetLineInventorySummary` (Assembly scope), `Item_ListForCutoverLocation`, `Item_Update` (type gate), and Ignition `Parts/Item`, Item Master + `ItemMaster/Identity` (Box Quantity enabled by the NAME "Pass-Through" -- a name-only rename would break it), `ItemMaster/Boms`, `Location/AttributeOptions`. Full release contract applies.
...
**Last updated:** 2026-09-18 -- **RELEASED TO PROD: migrations `0090`-`0095` + a 136-resource Ignition bundle (everything pending since `192c77c1`, plus `hunter/explore`). SQL committed clean 13:23 ET; Ignition imported and verified against git. Runbook + outcome: `notes/2026-09-18_prod-release-runbook-bundle.md`.**

> ### 2026-09-18 bundle release
>
> **Now on prod:** OEE-enabled locations + availability roll-up (`0090`); Line Inventory panel docked on the six M&A pages, Tolerances, Box Quantity (`0091`/`0094`/`0095`, tray projection retired); held stock no longer counts against a line's Max (`Lot_Create` v1.7); Assembly OUT elevated shipping-label reprint (`0093`) **and** Hunter's operator reprint + print-failure-banner reprint; Hunter's 30-min downtime-reason grace window; die cast shift confirmation (shift starts blank); released basket Good shows "-"; die cast screen resets whenever the press changes / on return (`resetForPress` + `custom.watchedCellId`); duplicate toasts on die cast + AIM Pool Config removed; cutover location-first; Trim OUT compact layout; tool shot-count correction; **mutation attribution** (`df99b8d5`, no DEV-user fallback) plus its missed call sites (`c8a18322`: 8 downtime-popup calls + Hunter's two reprint buttons); Assembly OUT camera is `ia.display.iframe` and fills its panel on both screens; Jacques's direct prod Designer edits pulled into git first (CameraPane, Tools collapsible header, OperatorEditor 500 px).
>
> **Numbers:** Execute at `e3e0aa25`, fingerprint `f5e07bd04140`, backup `MPP_MES_Prod_pre-release_0089_20260918_132322.bak` (verified), COMMITTED in 11.0 s (prod rehearsal lock window 9.3 s under live traffic; ProdSim 0.4 s). SQL suite 3840/3840. Dev smoke-tested (attribution via PIN 00006, grace window, die cast confirm with no write, line inventory, trim, cutover, OEE pre-flight batch).
>
> **Prod Ignition, 13:49 export vs git `e3e0aa25`:** every shipped resource identical. Still to delete by hand (harmless): Core NQ `workorder/Assembly_GetComponentProjection`, MPP view `Components/PlantFloor/ComponentProjectionRow`; leftovers `DieCastEntry/RejectPanel`, Core `BlueRidge/temp`. Intentionally different: AIM timers enabled on prod, MPP_Config `global-props` (prod datasource `mpp`), Jacques's prod-only `MachiningIn` sizing tweak.
>
> **Lessons:** (1) paste the fingerprint (typo aborted the first Execute, again); (2) import archives are easy to cross -- an MPP_Config archive went into Core (29 views, removed by hand) and a full-project export was nearly imported; check the dialog lists only what `CONTENTS.txt` says; (3) **a view nested inside another view's folder cannot be imported** (`Popups/PausedLotList/PausedLotRow`: "found non-folder in the way") -- prod has never had it; (4) the prod gateway had been edited directly (Hunter 09-16, Jacques 09-14..09-18) -- always diff a fresh prod export against git before building the archives; (5) a Designer save pushes to open sessions -- no F5 round needed.
>
> **Config Tool attribution:** MPP_Config requires AD login; a login with no matching active `AppUser.AdAccount` is refused on save ("Not saved"). Keep every Config Tool user's AD Account equal to their exact login name. Dev: `JGP` (Id 22) AD Account set to `admin` (the Dev Designer login) via `AppUser_Update`.
>
> **Follow-ups (not in this release):** part-type recategorization (OPEN TODO at top); move `PausedLotRow` to a standalone path; unmapped AD user gets a second "Action failed" toast (view should stop after the attribution refusal); Line Inventory dock + Tolerances error on the Fallback Terminal before a location is chosen (the two procs return no result set when the location has no line); AssemblyNonSerialized header overflows at 1200 px; flag the three 6MA Cam Holder Line 1 conveyors OEE-enabled (line stays flagged -> roll-up) and check `Machining Conveyor`'s location type; `Build-ChangeExport` gained `-ExcludeResource` / `-IncludeResource`.

**Previously:** 2026-09-17 (evening) -- **Assembly OUT: elevated shipping-label reprint (handoff section 2). One new read proc, two new views, a footer bar on both Assembly OUT screens; no migration. Proc tested and on Dev; views scanned; NOT live-verified -- needs a PIN sign-in + a real AD elevation, which the dev gateway cannot do. Not deployed.**
...
**Previously:** 2026-09-17 -- **Trim OUT compact layout: scrap tiles wrap, the scrap lists scroll inside their boxes, and the buttons stay visible. Four MPP views, no SQL. Browser-verified and signed off by Jacques; not deployed -- release handoff in `notes/2026-09-17_prod-release-handoff-trim-out-layout.md`.**
...
> ## ✅ Line Inventory panel — built 2026-09-18, **released to prod 2026-09-18** (see the bundle entry above; Dev now has `0088`/`0089`)
>
