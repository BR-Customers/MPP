...
| 2.4 | 2026-09-11 | Blue Ridge Automation | **An open container belongs to a station, not just a line.** (migration `0078_container_station`) `Lots.Container.StationLocationId BIGINT NULL FK → Location.Id` — the terminal filling the box. Open-box resolution in `Workorder.Assembly_CompleteTray` v1.4 is (line, part, **station**): the station's own box, else claim an unowned one, else open one owned by the station; the full-box guard uses the same resolution, so one station's full box never blocks another. A NULL or non-Terminal `@TerminalLocationId` keeps the pre-`0078` line-wide rule. `Lots.Container_GetOpenByCell` v1.1 gains optional `@StationLocationId` (own + unowned boxes) and `@ClosureMethod` filters and returns `StationLocationId` / `StationCode`. **Why:** line `MA2-6MACH` carries three assembly-out terminals (METTs A, METTs B — both ByCount, each with its own printer — and the vision cell); the two METTs stations run the same part numbers at the same time into separate boxes and each switches between several part boxes, which a (line, part) key merged into one container, one AIM serial and one label. The §Container table above predates Arc 2 and still lists columns the live table does not carry (`ContainerName`, `LotId`, `AimShipperId`, `HoldNumber`); only the `StationLocationId` row is new here. |
| 2.3 | 2026-09-10 | Blue Ridge Automation | **Cavity identity moves from a die-wide integer ordinal to a per-part alphabetic code.** (spec `docs/superpowers/specs/2026-09-10-cavity-alpha-code-design.md`, migration `0076_toolcavity_alpha_code`) MPP runs family dies — one 12-cavity die casting four part numbers — and names a cavity by a **letter scoped to the part it cuts** (*"6MA EX 1 cavity a"*, per migration `0072`'s own header). The letter was already being hand-typed into the free-text `ToolCavity.Description` (`In 1 Da` / `Db` / `Dc` on prod) because the schema had nowhere else for it, inconsistently and invisibly to every query. **(1)** `Tools.ToolCavity.CavityNumber INT` → **`CavityCode NVARCHAR(4) NOT NULL`**, 1–4 lowercase letters, immutable once saved. **(2)** Uniqueness re-scoped from `(ToolId, CavityNumber)` to **`UQ_ToolCavity_ActiveToolItemCode (ToolId, ItemId, CavityCode)`** filtered on `DeprecatedAt IS NULL`, so a family die carries four cavities called `a`, one per part. `ItemId` stays NULLable — SQL Server compares NULLs as equal for uniqueness, so a non-family die's cavities form one group and still letter correctly. **(3)** `Lots.Lot.CavityNumber` **dropped** and the D2 manual-cavity fallback retired with it: `@ToolCavityId` is now unconditionally required for a die-cast-origin LOT (`Lots.Lot_Create`). **(4)** `Lots.Lot_Get`'s result alias `ToolCavityNumber` → **`ToolCavityCode`**. **(5)** Seven read procs now order `PartNumber, CavityCode` — without the part key a 12-cavity/4-part die renders `a,a,a,a,b,b,b,b` on the config grid, the Bulk-Open grid, the shift-output rows and the right-rail tally. **Migration guards, both pre-write:** a `(Tool, Item)` group larger than 26 aborts; and a **family die that still has an unmapped cavity aborts**, because that row drops into the NULL group alone and shifts its peers' letters — prod's `DMO124` cavity 7 was the live instance. A third check is advisory only, printing where the derived letter disagrees with the one already at the end of `Description`; it prints rather than aborts because the pattern is a heuristic over free text and a per-die-half naming scheme legitimately disagrees. `ToolCavity.Description` is otherwise **unchanged** — still the display name per the 2026-08-19 decision. |
| 2.2 | 2026-09-03 | Blue Ridge Automation | **§7 Tools brought current — the shot-counter decision was reversed a month ago and this doc still carried the superseded rationale.** (1) **`Tools.Tool.ShotCount INT NOT NULL DEFAULT 0`** and **`ShotLimit INT NULL`** documented (migration `0050_tool_shot_count.sql`, 2026-08-04, FAT #26/#27). The pre-v2.2 text asserting *"No shot counter column — shot counts derive from `Workorder.ProductionEvent` group-by `(Tool, Cavity)`"* was **false as of `0050`** and is now marked as reversed rather than deleted: `ShotCount` is materialized and live-incremented by `Workorder.DieCastShiftOutput_Record` v1.2, with **no** event ledger and **no** reconcile job by design; derived remaining/percent/near/over come from **`Tools.ufn_ShotStatus`** (also previously undocumented, as was `Tools.Tool_GetShotStatusForCell`). (2) **`Tools.Tool_Duplicate`** (2026-08-18) documented, and its carried-vs-reset split promoted to a table in §7 — it is the authoritative definition of what counts as tool *configuration* (type, description, die rank, shot limit, cavities whole, attribute values) versus *per-physical-asset* state (code/name, status, shot count, mount history). (3) New normative rule: **cross-database tool copies resolve FKs by natural key, never by Id** — `ToolAttributeDefinition` ships empty and is authored per installation, and `Location.Id` was re-keyed in the 2026-07 reconcile, so an Id-based copy silently mis-attaches attribute values. Reference implementation `sql/scratch/2026-09-03_import_prod_tools_to_dev.sql` (proc-driven import of the customer's dies from `MPP_MES_Prod` into `MPP_MES_Dev`). No schema change in this revision — documentation catching up to migrations `0050` + the `Tool_Duplicate` repeatable. |
| 2.x | 2026-09-02 | Blue Ridge Automation | **`Location.AppUser` gains `Pin NVARCHAR(5) NOT NULL UNIQUE`** (migration `0069_appuser_pin.sql`) — the 5-digit terminal sign-in identifier that replaces initials as the login key. Plaintext by design: an identifier, not a credential. `NOT NULL` for both identity classes (an AD user must exist here for elevation to resolve, and may also sign in at a terminal), which removes the operator-vs-interactive branch from the sign-in path. UNIQUE is deliberately unfiltered so a retired person's PIN is never reissued. New CHECK `CK_AppUser_Pin_Format` enforces exactly five numeric characters; **leading zeros are significant** (full-time `04218` vs temp `40218`), hence NVARCHAR and string-typed parameters throughout. `Initials` and `DisplayName` are unchanged and remain the attribution/display fields. Existing rows backfilled with zero-padded Id placeholders; there is no seed roster — operators self-provision at the terminal. |
| 2.1 | 2026-07-29 | Blue Ridge Automation | **Die Cast per-cavity Open → accumulate → release lifecycle.** (spec `docs/superpowers/specs/2026-07-28-diecast-per-cavity-lifecycle-design.md`, migration `0045_diecast_per_cavity_lifecycle`) Die-cast entry is redesigned from a one-basket-per-create origin mint into a per-cavity accumulator: each `(Tool, ToolCavity)` owns one independent `Open` basket that fills across operators/shifts and is released to storage only when the operator closes it. **(1)** New fifth `Lots.LotStatusCode` row **`Open`** (`BlocksProduction = 0`, appended `Id` — statuses 1-4 unchanged): the LOT exists and accumulates at the press but is not yet on its route. `Lot_GetWipQueueByLocation` excludes it (`NOT IN ('Closed','Open')`). **(2)** New **`Workorder.DieCastContribution`** table (§4) — an append-only per-shift net-good ledger, one row per shift-output submission per LOT. **(3)** New procs `Lots.DieCastLot_Open` (mint the per-cavity Open basket, `PieceCount = 0`), `Workorder.DieCast_GetShiftOutputBreakdown` (splits a die-wide gross shot count across a cavity's LOTs — a closed LOT keeps its credited good, the open LOT gets the remainder floored at 0), `Workorder.DieCastShiftOutput_Record` (per-cavity net-good contribution + additive scrap + shot-loss fan-out), `Lots.DieCastLot_Release` (`Open → Good` + move to WHSE storage — the LOT's first route movement), `Lots.DieCastLot_Void` (empty basket `Open → Scrap`), `Lots.Lot_GetOpenByTool` (running open-basket list). **(4)** Reworked `Lot_GetShiftCavityTally` to be additive-correct (`PieceSum = SUM(PieceCount)` good-only, no scrap add-back — die-cast scrap has been additive since migration `0042`, the old add-back was double-counting) and `Lot_GetAttributeHistory` (new `Contribution` stream over `DieCastContribution`). **Locked decisions:** basket holds GOOD only (scrap is a separate additive metric, never decrements `PieceCount`); operator enters GROSS shots, system computes good = gross − scrap − shot-losses; accumulation is shift-scoped via `Oee.Shift`; `Item.MaxLotSize` is a **soft** basket ceiling (warns, never blocks); `Lots.v_LotDerivedQuantities.TotalInProcess = 0` throughout accumulation (not on-route yet). Full `MPP_MES_Test` suite 2225/0; deployed to `MPP_MES_Dev`; `DieCastBody` view rebuilt on the new lifecycle (first cut, pending live smoke — see `PROJECT_STATUS.md` 2026-07-29). |
...
Normative for all Arc-2 event-table DDL; governs `0020_arc2_phase1_shop_floor_foundation.sql`. Full design + rationale: `docs/superpowers/specs/2026-06-09-arc2-phase1-sql-foundation-design.md`; decision provenance: `Meeting_Notes/2026-06-08_Phase0_Decision_Log.md`.

- **B2 Partitioning:** ~14 high-volume Arc-2 event tables use **monthly `RANGE RIGHT` partitioning** + sliding-window retention. Partition function (`pf_MonthlyUtc`) + scheme (`ps_MonthlyUtc`, single `PRIMARY` filegroup) created in `0020` — cannot retrofit to populated tables. **Sliding-window mechanism is `TRUNCATE TABLE … WITH (PARTITIONS(…))`** (destructive age-out, no archive-to-coldstore in MVP), which imposes **no aligned-unique-index requirement** — so the project's singleton `BIGINT IDENTITY Id` PK convention is preserved (clustered index is the partition-aligned hot path, e.g. `(LotId, EventAt)`; `Id` stays a `NONCLUSTERED` PK). Excludes `Oee.DowntimeEvent` (Phase 8) and `Quality.HoldEvent` (Phase 7) — each is partitioned in its owning phase's migration.
...
- **B4 Genealogy closure:** `Lots.LotGenealogy` materialized as a **closure table** (`LotGenealogyClosure`: ancestor/descendant/depth) maintained by genealogy procs — O(1) Honda trace. Created in `0020`; `Lot_Create` writes the self-row (`Depth=0`); split/merge maintenance is Phase 2.
...
- **B6 Identifier sequence:** `Lots.IdentifierSequence_Next` is a **row-locked table** (not SQL `SEQUENCE`) — gap-free, format-controlled, reseedable, 9,999,999 rollover. **Cutover seed ≥ ~3,000,000** (integration constraint); **MESL/MESI** format retained; exact per-sequence seeds set at cutover (owed from Ben — a cutover gate, not a build gate).
- **B7 OperationLog split:** `Audit.OperationLog` → 7-yr general `OperationLog` + 20-yr `Lots.LotEventLog` (both created/repartitioned in `0020`; lot-relevant events route to `LotEventLog` via `Audit_LogOperation`).
...
- **B1 Retention:** per the FDS §11 retention table (20-yr Honda traceability vs 7-yr general operational/audit), enforced via the B2 sliding-window. Subject to MPP IT confirmation.
...
- **Operator** rows — `AdAccount` NULL, `IgnitionRole` NULL. Identified by a 5-digit `Pin` entered at a terminal; no authentication. Self-provisioned at the terminal on first unrecognised PIN (FDS-04-010); corrected or deprecated via the Configuration Tool Admin screen.
- **Interactive User** rows (Quality, Supervisor, Engineering, Admin) — `AdAccount` NOT NULL, `IgnitionRole` NOT NULL. Authenticate via Active Directory for elevation, and carry a `Pin` like everyone else for terminal presence.

`Pin` is `NOT NULL` for **both** classes: an interactive user must exist here for elevation to resolve their account, and may also sign in at a terminal, so a universal PIN removes any operator-vs-interactive branching from sign-in.
...
- `IgnitionRole IS NULL OR AdAccount IS NOT NULL` (`CK_AppUser_IgnitionRole_Requires_AdAccount`) — an Operator (no AD) cannot carry an Ignition role; roles apply only to AD-backed users.
- `LEN(Pin) = 5 AND Pin NOT LIKE '%[^0-9]%'` (`CK_AppUser_Pin_Format`) — exactly five numeric characters. A leading zero satisfies this; a 4-character value does not.

**Legacy columns — removed.** `ClockNumber NVARCHAR(20)` and `PinHash NVARCHAR(255)` were dropped by migration `0011_drop_appuser_legacy_auth.sql`, along with the `AppUser_SetPin` and `AppUser_GetByClockNumber` procs. Note that the new `Pin` column is **not** a revival of `PinHash`: that was a hashed *credential* for the retired clock-number auth model, whereas `Pin` is a plaintext *identifier* that grants no privilege.
...
>
> **v2.1 changes (2026-07-29) — per-cavity Open → accumulate → release lifecycle.** Die-cast LOT creation is no longer a single `Good`-status origin mint. `Lots.DieCastLot_Open` mints one `Open`-status basket LOT per `(ToolId, ToolCavityId)` at `PieceCount = 0` when the operator scans a fresh pre-printed LTT — at most one `Open` LOT per (tool, cavity) at a time. The basket then **accumulates**: each shift, `Workorder.DieCastShiftOutput_Record` credits net-good pieces onto the LOT (raising `PieceCount`) via a `Workorder.DieCastContribution` row, while scrap posts additively to `Workorder.RejectEvent` and never touches `PieceCount`. While `Open`, the LOT is deliberately **not on its route** — `Lots.v_LotDerivedQuantities.TotalInProcess = 0` and `Lot_GetWipQueueByLocation` excludes it. The operator **releases** the basket (`Lots.DieCastLot_Release`) when it's full: `LotStatusId` transitions `Open → Good` and the LOT gets its first `LotMovement` (cell → warehouse storage), which is what makes it appear in the Trim IN queue for the first time. An empty `Open` basket may instead be **voided** (`Lots.DieCastLot_Void`, `Open → Scrap`), freeing the cavity/LTT. `Item.MaxLotSize` (`PartsPerBasket`) governs this basket exactly as before, but as a **soft** ceiling — the release/accumulate procs warn on overflow, they never reject it.
...
| RfidTag | NVARCHAR(100) | NULL | Reserved placeholder for a future RFID-encoding phase (migration 0070). Not populated or read anywhere yet. |
...
> - `ProductionEvent`, `ProductionEventValue`, `ConsumptionEvent`, `RejectEvent`, `ScrapSource`, `DieCastContribution` — **MVP** (Production Data Acquisition is included and expanded)
...
**Shot counting — reversed at v2.2 (migration `0050`, FAT #26/#27).** Pre-v2.2 this section read *"No shot counter column — shot counts derive from `Workorder.ProductionEvent` group-by `(Tool, Cavity)`"*, on the rationale that a materialized aggregate risks drift against the event stream. That decision was **reversed**: `Tool.ShotCount` is now a materialized lifetime counter incremented live by `Workorder.DieCastShiftOutput_Record`, with `Tool.ShotLimit` as the optional design life. Deliberately **no event ledger and no reconcile job** (spec `docs/superpowers/specs/2026-08-04-tool-shot-count-design.md` §2) — the die-cast write proc is the single writer. Derived remaining / percent / near-limit / over-limit are computed on read by **`Tools.ufn_ShotStatus`**, surfaced per cell by `Tools.Tool_GetShotStatusForCell` and reported by the *Die Cast Shot Count* PDF report.

**Configuration vs. per-physical-asset state.** `Tools.Tool_Duplicate` (added v2.2, 2026-08-18) is the authoritative statement of the split, and any tool-copy path — including a cross-database import — should honour it:

| Carried (describes the tool's *design*) | Reset (belongs to the *physical asset*) |
|---|---|
| `ToolTypeId`, `Description`, `DieRankId`, `ShotLimit` | `Code`, `Name` (operator-supplied — identity is never cloned) |
| every non-deprecated `ToolCavity`, **whole** (number + description + status — a Closed/Scrapped cavity is a decision about the die design, not wear) | `StatusCodeId` → forced `Active`; `ShotCount` → 0; `CreatedAt` / `CreatedByUserId` stamped fresh; `DeprecatedAt` → NULL |
| every `ToolAttribute` whose definition is still active | `ToolAssignment` — mount history is per-asset, and `UQ_ToolAssignment_ActiveCell` would reject it anyway |

**Cross-database copies must resolve FKs by natural key, never by Id.** `ToolAttributeDefinition` ships empty and is authored per installation, and `Location.Id` was re-keyed in the 2026-07 location reconcile — so `ToolType.Code`, `(ToolType.Code, ToolAttributeDefinition.Code)`, `DieRank.Code` and `Location.Code` are the only safe joins between two MES databases. An Id-based `INSERT … SELECT` will silently attach attribute values to the wrong definitions. Reference implementation: `sql/scratch/2026-09-03_import_prod_tools_to_dev.sql`.
...
| CavityCode | NVARCHAR(4) | NOT NULL | Per-part cavity identifier: 1–4 lowercase letters (`a`, `b`, `c`…). A 12-cavity family die casting four parts carries four cavities called `a`, one per part — which is how MPP names them (*"6MA EX 1 cavity a"*). Immutable once saved. Replaced the die-wide integer ordinal in migration `0076`. |
...
**Unique:** `UQ_ToolCavity_ActiveToolItemCode` — filtered UNIQUE `(ToolId, ItemId, CavityCode)` where `DeprecatedAt IS NULL`. Scoped **per part**, not per die: `ItemId` stays NULLable, and because SQL Server compares NULLs as equal for uniqueness a non-family die's cavities form one group and still get distinct letters.
