...
async function releaseRow(cavityName) {
  ...
  await sleep(1200);
  // Walk UP from each Release button only as far as a ROW. Going further
  // reaches the container that holds every row, whose text contains every
  // cavity name -- so the first button always "matched" and the same basket
  // got released twice.
  const res = await evalJs(cdp, [
    ...
    '      if (t.includes(want)) { b.click(); return "clicked"; }',
...
// The fixture reset happened out of band, so the session is still holding the
// pre-reset list until something re-reads.
await clickButton(cdp, 'Refresh', { settle: 3500 });
