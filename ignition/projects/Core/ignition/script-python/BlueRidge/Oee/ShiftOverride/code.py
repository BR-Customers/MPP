...
# Binding-safe empty shape for the Supervisor Dashboard's plant roll-up tile.
_EMPTY_PLANT_AVAILABILITY = {
    "ShiftId": None, "ScheduleName": "", "EquipmentCount": 0,
    "PlannedMinutes": 0, "RunMinutes": 0, "DowntimeMinutes": 0,
    "Availability": None, "AvailabilityPct": "", "HasData": False,
}


def plantAvailabilityOrEmpty(_refreshToken=None):
    """Plant-wide availability for the currently-OPEN shift, for the Supervisor
       Dashboard tile. ALWAYS a fully-shaped dict, never None.

       Oee.Shift_GetAvailability returns ONE ROW PER PIECE OF EQUIPMENT and has no
       plant-level row, so the roll-up happens here:

           Availability = SUM(RunMinutes) / SUM(PlannedMinutes)

       That is a MINUTES-WEIGHTED roll-up, deliberately NOT the average of the
       per-equipment percentages -- a press that only ran ten minutes would
       otherwise swing the plant figure as hard as one that ran the whole shift.
       It is also override-aware for free, because each equipment row's
       PlannedMinutes already reflects any Shift Override on that equipment.

       HasData is False when no shift is open or no equipment resolved, so the tile
       can say so instead of printing a misleading 0%.
    ...
    try:
        shift = BlueRidge.Oee.Shift.getOpen()
        if not shift or shift.get("Id") is None:
            return dict(_EMPTY_PLANT_AVAILABILITY)

        rows = availability(shift.get("Id"), None)
        if not rows:
            out = dict(_EMPTY_PLANT_AVAILABILITY)
            out["ShiftId"] = shift.get("Id")
            out["ScheduleName"] = shift.get("ScheduleName") or ""
            return out
        ...
        downtime = 0.0
        for r in rows:
            planned += float(r.get("PlannedMinutes") or 0)
            run += float(r.get("RunMinutes") or 0)
            downtime += float(r.get("DowntimeMinutes") or 0)

        avail = (run / planned) if planned > 0 else None
        return {
            "ShiftId": shift.get("Id"),
            "ScheduleName": shift.get("ScheduleName") or "",
            "EquipmentCount": len(rows),
            "PlannedMinutes": int(planned),
            "RunMinutes": int(run),
            "DowntimeMinutes": int(downtime),
            "Availability": avail,
            "AvailabilityPct": "" if avail is None else "%.1f%%" % (avail * 100.0),
            "HasData": planned > 0,
        }
    except Exception, e:
        BlueRidge.Common.Util.log("plantAvailabilityOrEmpty failed: %s" % e, level="error")
        return dict(_EMPTY_PLANT_AVAILABILITY)
