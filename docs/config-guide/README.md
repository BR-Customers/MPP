...
- **Shift Overrides** — the screen has no sidebar entry in the app (reachable only at
  `/shift-overrides`). The section carries a warning callout saying so; remove it once a nav
  item is added.
