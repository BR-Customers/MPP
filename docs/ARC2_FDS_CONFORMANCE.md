...
| FDS-10-011 Hold/CRT release | SHALL | Partial → **Partial (improved)** | `Hold_Release:20,70-115`; `Lot_SetCrt:27`; `LotDetail.doCrtToggle` + `crtToggleRequested` handler; `Session._ELEVATED_REPLAY_MESSAGES:180` | CRT-release proc exists, not yet a `ReleaseDispositionCode`. **CRT half of the elevation gap is CLOSED** — the LOT Detail CRT toggle is AD-elevation-gated in both directions via `Session.requireElevation(..., "CrtToggle", ...)` with a `crtToggleRequested` replay. **Hold** release is still ungated (P7-1). |
| FDS-10-012 CRT lifecycle | SHALL | Missing → **Built** | `0037:16-24`; `Lot_SetCrt:109-113` (`CrtActive=1`), `Lot_ClearCrt`, `Crt_GetRequiredInspections`, `Crt_FlagMissedInspection`; test `040_Crt_workflow`; `LotDetail` CRT badge + `BtnCrtToggle` | **P9-2 resolved** (the single most-stale 🔴 row); codes seeded (LogEventType 63-66). **P9-6 UI badge caveat closed** — LotDetail carries the CRT badge plus an elevation-gated set/clear toggle that mutates `view.params.lotId`. |
