...
| 6 | [`MPP_MES_ERD.html`](./MPP_MES_ERD.html) | Interactive ERD generated from the live `MPP_MES_Dev` schema by SchemaGen — per-schema + cross-schema tabs, stored-procedure dependency graphs, DB-sourced table/column documentation. Open in a browser; regenerate per [Regenerating the ERD](#regenerating-the-erd) |
...
├── MPP_MES_ERD.html                    Interactive ERD (generated by SchemaGen from MPP_MES_Dev — do not hand-edit)
...
`MPP_MES_ERD.html` is **generated**, not hand-authored — do not edit it. It is produced by
**SchemaGen** (`../SchemaGen`, a separate Blue Ridge repo) reading schema metadata out of the
live `MPP_MES_Dev` database. Generation is read-only: `sys.*` catalog views only, no user data,
no locks.

```bash
cd ../SchemaGen
python generate_erd.py --config mpp.json
cp mpp-data-model.html ../MPP/MPP_MES_ERD.html
```

`mpp.json` carries the connection (localhost, trusted connection) and the filters: `dbo` and
`test` schemas excluded, `schema_prefix` grouping at 30 tables/tab, views included, stored
procedures and cross-schema tabs on.

For an air-gapped machine or a customer documentation package, add `--embed-assets` — it inlines
Mermaid and JSZip (verified against their SRI hashes) so the page renders with no network. The
committed copy is the CDN build, which is ~3x smaller.

**What the ERD now shows:** one tab per schema (Audit, Location, Lots, Oee, Parts, Quality,
Tools, Workorder), a cross-schema overview plus a focus tab per schema, per-tab stored-procedure
and table-dependency graphs, and a Documentation section per tab carrying table and column
comments read from SQL Server extended properties. SVG and ZIP export per tab; dark / light /
print themes.

**What it no longer shows:** MVP / CONDITIONAL / FUTURE scope badges. The previous
hand-authored ERD tagged tables by scope; SchemaGen reads only database metadata and has no
notion of project scope. `reference/MPP_Scope_Matrix.xlsx` is the scope authority.

**Keeping the documentation section useful:** table and column comments come from
`MS_Description` extended properties in the database, applied by the SQL migrations. A column
with no extended property shows an empty comment cell — so improving the ERD's documentation
means adding extended properties in SQL, not editing the HTML.
