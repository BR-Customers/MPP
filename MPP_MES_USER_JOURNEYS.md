...
| 0.9-rev2 | 2026-05-04 | Blue Ridge Automation | **Operator access model clarification + die cast screenshot fix.** Added Arc 2 access model statement: operator terminals show only their machine workflow screen; Plant Floor Home, LOT Search, LOT Detail, and Genealogy Lookup are supervisor / elevated views. Die Cast scene: removed the Plant Floor Home — Location Details screenshot (supervisor view, not what Carlos sees); moved the Die Cast LOT Entry screenshot immediately after the initials-entry screen. Trim error state caption updated — "Open LOT Detail" removed as an operator action (LOT Detail is elevated access). LOT Detail caption flagged as elevated. |
