...
*Check OUT state: OperationTemplate banner (TrimOut · 5G0-TRIM-4102 v3 — "whole-LOT move, no split at Trim"). Closing ProductionEvent captures Operator Initials; Reject Entry · Trim Yield Loss records RejectEvents against the LOT with Trim-area Defect Code and Quantity. The Destination — whole-LOT move panel selects one Machining line (Pieces locked at the full count); Submit · TrimOut + Move.*
...
![Machining OUT · Sub-LOT Split — 6MA sublotting line, 2 destinations](<mockup_screenshots/Screenshot 2026-04-30 124742.png>)

*Machining OUT · Sub-LOT Split (sublotting line — 6MA Cam Holder, RequiresSubLotSplit = ON). Closing ProductionEvent + the Sub-LOT Distribution panel: each row is a destination sub-LOT — operator enters Pieces and selects a destination; + Add sub-LOT adds a row. Submit · MachiningOut + Split + Route commits the split + route in one transaction. Non-sublotting lines auto-move whole to the coupled Assembly Cell (no screen).*
