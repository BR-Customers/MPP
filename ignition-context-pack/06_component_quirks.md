...
## Barcode scanning — the component is a KEYBOARD listener; the camera is a native action

Two completely different mechanisms, and the names mislead. Verified on 8.3.5 by reading the shipped client bundle (`/res/perspective/js/PerspectiveComponents.*.js` and `PerspectiveClient.*.js`) and by a real scan.

### `ia.input.barcodescannerinput` — wedge scanners only

Note the component id: **one word, no dashes or dots after `input`**. `ia.input.barcode-scanner` renders "component not found" — the same trap as `numeric-entry` vs `numeric-entry-field`.

Props: `regex`, `window` (default 20), `prefix`, `suffix`, `captureMode` (default `"keypress"`), `data`, `dataStyle`.

Two things make it look broken when it is merely unconfigured:

1. **`captureMode` is a DOM event name**, passed straight to `document.addEventListener(captureMode, handler, false)`. `keypress` / `keydown` / `keyup` are the meaningful values. It never touches a camera.
2. **With no `prefix`+`suffix` pair and no `regex`, there is no handler at all:**

```js
if (prefix.length > 0 && suffix.length > 0) return new PrefixSuffixHandler(prefix, suffix)
if (regex.length > 0) try { return new RegexHandler(new RegExp(regex), window) } catch (e) {}
return null          // ← default props land here: captures nothing, forever, silently
```

Also **`data` is append-only** — a scan does `props.write("data", data.concat(scanned))` and fires `onActionPerformed`. So a binding on `data[0]` shows the *first* scan ever and then never changes. Bind the last element, and clear the array between scans.

### `native/barcode` — the camera, and only inside the mobile App

The camera is an **event action**, not a component. Attach an action of type `native/barcode` to any component:

```json
{ "type": "native/barcode", "scope": "C",
  "config": { "config":  { "cameraPreference": "BACK", "type": "ANY",
                           "backgroundColor": "AUTO", "uuid": "<per-action>" },
              "context": { "screen": "...", "field": "..." } } }
```

It calls `window.__mobileInterface.launchAction(...)`. **That bridge exists only in the vendor's native mobile App.** In a desktop or mobile *browser* the client logs *"Native mobile action requested in non-mobile client. Action request ignored!"* and nothing happens — no error, no UI feedback. Check with `typeof window.__mobileInterface` before blaming your config.

Sibling native actions on the same bridge: `native/picture`, `native/geolocation`, `native/deviceId`, `native/gpsPermission`, `native/ndef`, `native/bluetooth`, `native/accelerometer`.

### The scan does NOT return to the component that asked for it

There is **one project-wide session event** for every scan in the app:

```python
def onBarcodeDataReceived(session, data, context):
    # data    = {"barcodeType": "qrcode", "text": "<payload>", "timestamp": <ms>}
    # context = the firing action's "context" object, echoed back verbatim
```

So `context` is the **only** thing that tells you which field the operator was scanning into. Give every action a `context` (e.g. `{"screen", "field"}`), keep the session event a one-liner into a dispatcher project script, and **drop context-less scans with a log line** rather than guessing a destination — writing a stray scan into whatever field was last focused is worse than ignoring it.

Note this handler runs in **gateway scope with no UI attached**, so an exception there is invisible to the operator; make it never-throw.

### Testing a scan without a device

The client exposes the inbound path the App uses. From the browser console of a live session:

```js
window.__webInterface.submitData({ type: 'native/barcode',
  data:    { barcodeType: 'code128', text: 'TEST-0001', timestamp: Date.now() },
  context: { screen: 'myscreen', field: 'myfield' } });
```

This drives the real session event end-to-end, so routing and state writes can be verified on a desktop before anyone installs the App.
