...
| 5 | `MPP_MES_ERD.html` | Interactive ERD — **generated from the live `MPP_MES_Dev` schema by SchemaGen**, not hand-authored. One tab per schema + 9 cross-schema tabs; per-tab stored-procedure/table-dependency graphs; table + column documentation pulled from SQL Server extended properties; pan/zoom, SVG/ZIP export, dark/light/print themes. Regenerate with `python generate_erd.py --config mpp.json` from `../SchemaGen` (see README § Regenerating the ERD). **Carries no MVP/CONDITIONAL/FUTURE scope badges** — the old hand-authored ERD did; scope lives in `reference/MPP_Scope_Matrix.xlsx`. | Visual reference for schema relationships |
...
- **PLC/OPC:** OmniServer (scales), direct native Allen-Bradley Ethernet/IP driver connections for tray-inspection/cam-holder assembly PLCs (`TrayInspectionStation` family — confirmed via decoded `.RSS` ladders for `6MA_CH`, `6C2_6MA_OilPan`, `5J6_OilPan`, `5K8_64A_OilPan`; **TOPServer is not used** — legacy's TOPServer-aliased tag names carry no address information for a direct connection, so any station not yet decoded this way has no confirmed real addresses), Cognex (vision). MIP handshake for serialized lines.
...
- **Model Y (mint-step placement):** a consume-mint is the **final route step of the *consumed* part**. A casting's route runs `DieCast→TrimIn→TrimOut→MachiningIn→` its consume-mint — `MachiningOut` when the line has a Machining OUT terminal (which **mints** a SubAssembly by consuming the casting), otherwise `AssemblyOut` (which mints the finished good directly). A SubAssembly identity exists only when the line has a Machining OUT terminal (decision C — pure route authoring).
- **Post-birth routes are a single step** (the shape seeded by `sql/seeds/029_seed_item_routes.sql`, which is the authority): a minted **SubAssembly**'s route is one `MachiningOut` step; a **FinishedGood**'s route is one `AssemblyOut` step. Both are ConsumeMint roles, so under `Lot_GetWipQueueByLocation`'s pending rule they stay pending until the LOT closes.
  > ⚠️ **Known wart (verified 2026-08-12, not yet fixed).** Because `ConsumeMint` is *unconditionally* pending, a minted SubAssembly whose only route step is `MachiningOut` never leaves the Machining OUT queue — it appears in `mapMachiningOutQueue`'s pick-list alongside the castings still awaiting minting, so an operator can select an already-machined LOT as a mint source. Latent so far only because Dev carried just two SubAssemblies (`5G0-SA`, `12270-6NA-M`) with no live LOTs. Seeding the real MPP part list adds 26 more, so this will start showing. Fixing it means either giving the SubAssembly an `AssemblyIn`/`AssemblyOut` route instead, or excluding LOTs whose ConsumeMint step already produced them.
