...
IF NOT EXISTS (SELECT 1 FROM Parts.ItemLocation WHERE ItemId=2 AND LocationId=76 AND DeprecatedAt IS NULL)
    INSERT INTO Parts.ItemLocation (ItemId, LocationId, CreatedAt, IsConsumptionPoint) VALUES (2, 76, SYSUTCDATETIME(), 0);
GO
...
--     The 6B2 container (cell 47) is the operator-testable non-ser consumption demo.
-- =========================================================================
...
-- eligibility: components staged/consumed at cell 47; 6B2 (the assembly) is produced there (IsConsumptionPoint=0 so Assembly_ScanIn validates); a 6B2-MACH scan-in LOT staged at machining-out 78
IF NOT EXISTS (SELECT 1 FROM Parts.ItemLocation WHERE ItemId=@B2M AND LocationId=47 AND DeprecatedAt IS NULL) INSERT INTO Parts.ItemLocation (ItemId,LocationId,CreatedAt,IsConsumptionPoint) VALUES (@B2M,47,SYSUTCDATETIME(),0);
IF NOT EXISTS (SELECT 1 FROM Parts.ItemLocation WHERE ItemId=@B2P AND LocationId=47 AND DeprecatedAt IS NULL) INSERT INTO Parts.ItemLocation (ItemId,LocationId,CreatedAt,IsConsumptionPoint) VALUES (@B2P,47,SYSUTCDATETIME(),0);
IF NOT EXISTS (SELECT 1 FROM Parts.ItemLocation WHERE ItemId=@B2  AND LocationId=47 AND DeprecatedAt IS NULL) INSERT INTO Parts.ItemLocation (ItemId,LocationId,CreatedAt,IsConsumptionPoint) VALUES (@B2,47,SYSUTCDATETIME(),0);
IF NOT EXISTS (SELECT 1 FROM Parts.ItemLocation WHERE ItemId=@B2M AND LocationId=78 AND DeprecatedAt IS NULL) INSERT INTO Parts.ItemLocation (ItemId,LocationId,CreatedAt,IsConsumptionPoint) VALUES (@B2M,78,SYSUTCDATETIME(),0);
-- stage the BOM components at cell 47 (4 trays x 24: need 96 6B2-MACH + 192 6B2-PIN) + a scan-in LOT at 78
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=@B2M,@LotOriginTypeId=1,@CurrentLocationId=47,@PieceCount=96, @AppUserId=@U,@LotName=N'SMK-6B2M-47';
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=@B2P,@LotOriginTypeId=1,@CurrentLocationId=47,@PieceCount=192,@AppUserId=@U,@LotName=N'SMK-6B2P-47';
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=@B2M,@LotOriginTypeId=1,@CurrentLocationId=78,@PieceCount=24, @AppUserId=@U,@LotName=N'SMK-6B2M-SCAN';

-- =========================================================================
-- 1. MACHINING IN queue: 3 whole 5G0-C cast LOTs at MA1-5GOF-MIN (76) -> renamed to 5G0-MACH on pick
-- =========================================================================
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=2,@LotOriginTypeId=1,@CurrentLocationId=76,@PieceCount=48,@AppUserId=@U,@LotName=N'SMK-MIN-1';
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=2,@LotOriginTypeId=1,@CurrentLocationId=76,@PieceCount=47,@AppUserId=@U,@LotName=N'SMK-MIN-2';
DELETE FROM @rLot; INSERT INTO @rLot EXEC Lots.Lot_Create @ItemId=2,@LotOriginTypeId=1,@CurrentLocationId=76,@PieceCount=48,@AppUserId=@U,@LotName=N'SMK-MIN-3';
...
-- =========================================================================
-- 4. ASSEMBLY NON-SERIALIZED open container (6B2, non-ser 4x24) at MA1-COMPBR-AOUT (47)
--    Components 6B2-MACH + 6B2-PIN staged at the cell (0.8); each tray close consumes them.
-- =========================================================================
DELETE FROM @rCon; INSERT INTO @rCon EXEC Lots.Container_Open @ItemId=@B2,@ContainerConfigId=@B2Cfg,@CellLocationId=47,@AppUserId=@U;
...
SELECT ViewName, [Pick / Enter], Expect, [Id to use] FROM (VALUES
  ...
  (4, N'Assembly Non-Serialized', N'cell  MA1-COMPBR-AOUT',           N'open 6B2 container (0 / 96), consumes 6B2-MACH + 6B2-PIN', @AnonCon),
  (11,N'Assembly IN',             N'scan LTT SMK-6B2M-SCAN at cell 47', N'moves the 6B2-MACH LOT into the cell queue',  CAST(NULL AS BIGINT)),
