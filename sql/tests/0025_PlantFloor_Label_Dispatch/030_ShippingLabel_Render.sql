...
--               part number, description, quantity, die-rank (D/C part level via
--               genealogy trace), MFG lot (AIM serial) and the composed 16-char serial
--               (13218001 supplier code + last 8 of the AIM serial). Asserts ASCII-only
--               and that no {tokens} leak through.
-- =============================================
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
VALUES (N'RNDFG1', @Item, @OrigMfg, @Good, 6, @Loc, 1);
...
INSERT INTO Lots.Lot (LotName, ItemId, LotOriginTypeId, LotStatusId, PieceCount, CurrentLocationId, CreatedByUserId)
VALUES (N'RNDFG2', @Item, @OrigMfg, @Good, 6, @Loc, 1);
...
DECLARE @HasLevel NVARCHAR(10) = CASE WHEN @Zpl LIKE N'%RND-A%' THEN N'1' ELSE N'0' END;
EXEC test.Assert_IsEqual @TestName = N'[Render] dc part level (die rank) present', @Expected = N'1', @Actual = @HasLevel;
