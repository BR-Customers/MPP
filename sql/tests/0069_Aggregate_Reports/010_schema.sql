...
-- defect codes, and any one of them that outlives its file would otherwise be
-- counted here (0011_Quality_Spec/040 did exactly that until its cleanup was
-- completed). Scoping keeps this asserting the SEED regardless of which tests
-- ran first, so a future fixture cannot re-break it. Same idiom as
-- 0046_Shift_Reconcile and 0062_Oee_ShiftAttribution. No seeded defect code
-- contains "TEST".
...
EXEC test.Assert_IsEqual @TestName = N'[0067/0084] exactly six codes flagged IsNonRejectScrap',
    @Expected = N'6', @Actual = @n;
